...
public class EstoqueDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Peca>(e =>
        {
            ...
            e.HasMany(p => p.Fornecedores).WithOne(e => e.Peca).HasForeignKey(e => e.Peca_Id).OnDelete(DeleteBehavior.Cascade);
            e.HasMany(p => p.Anexos).WithOne(a => a.Peca).HasForeignKey(a => a.Peca_Id).OnDelete(DeleteBehavior.Cascade);
            e.HasMany(p => p.Historicos).WithOne(h => h.Peca).HasForeignKey(h => h.Peca_Id).OnDelete(DeleteBehavior.Cascade);
        ...
        modelBuilder.Entity<PecaFornecedor>(e =>
        {
            ...
            e.HasOne(p => p.Fornecedor)
             .WithMany().HasForeignKey(p => p.Fornecedor_Id)
